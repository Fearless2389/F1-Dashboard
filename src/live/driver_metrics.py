...
def compute_metrics(driver_code: str, season: Optional[int] = None) -> dict:
    ...
    # ── Radar: Consistency ───────────────────────────────────────────────────
    # Lower stdev of finish position = more consistent. Map stdev 0..7 → 100..0.
    if len(finishes) >= 3:
    ...
    # ── Radar: Overtaking ────────────────────────────────────────────────────
    # Positions gained per race = grid_position - finish_position.
    # Median across the sample, mapped: +3 gained → 80, 0 → 50, -3 → 20
    if "grid_position" in sample.columns:
        ...
        if len(gainers) >= 3:
            delta = (gainers["grid_position"].astype(float)
                     - gainers["finish_position_clean"].astype(float)).median()
    ...
    # Proxy: per-race standard deviation of lap-time vs field benchmark.
    # We don't have per-lap deltas pre-baked, so we use DNF rate inversely
    # combined with consistency. Drivers that finish cleanly + consistently
    # = good tyre managers; drivers that DNF often = poor.
